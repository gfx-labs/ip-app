...
export const DepositWithdrawCollateralContent = (): JSX.Element => {
  ...
  return (
    <Box>
      ...
      { !isDepositType && collateralToken.ticker == 'MKR' && (
        <WarningAlert msg={MKR_WARNING_MSG}/>
      )}
      { !isDepositType && collateralToken.bpt && (
      ...
      <Box marginTop={2}>
        <DisableableModalButton
          ...
          disabled={disabled}
          onClick={() => {
            setType(isDepositType? ModalType.DepositCollateralConfirmation :
              ModalType.WithdrawCollateralConfirmation)
          }}
        />
      </Box>
      <Box
