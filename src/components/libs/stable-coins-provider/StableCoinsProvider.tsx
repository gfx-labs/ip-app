...
export const StableCoinsProvider = ({
  ...
  useEffect(() => {
    if (rolodex && rolodex?.addressUSDC) {
      getBalanceOf(currentAccount, rolodex.addressUSDC, USDC.decimals, rolodex.provider).then(
        (res) => {
  ...
  useEffect(() => {
    if (rolodex && rolodex?.addressUSDI) {
      getBalanceOf(currentAccount, rolodex.addressUSDI, USDI.decimals, rolodex.provider).then(
        (res) =>
