  ...
  const update = async () => {
    ...
    if (rolodex && rolodex.VC) {
      ...
      for (const [key, token] of Object.entries(tokens!)) {
        let p1 = getVaultTokenMetadata(vaultAddress!, token.address, rolodex!)
          .then((res) => {
          ...
          .then((res) => {
            if (res.livePrice) {
              ...
              token.vault_balance = Number(
                (token.vault_amount * token.value).toFixed(0)
              );
            }
          })
          .catch((e) => {
        ...
        px.push(p2);
        let p3 = getBalance(token.address, currentAccount, rolodex!.provider)
          .then((val) => {
